...
//
// Opens MWDX-render.log beside the executable and serialises writes from every
          ...
          const char* body) noexcept
{
    ...
    if (lvl == Level::Fatal) {
        ...
        if (g_fatalBoxShown.compare_exchange_strong(expected, true)) {
            ...
            MessageBoxW(nullptr, msg, L"dx9to11 - DirectX error",
...
void Init() noexcept
{
    ...
    if (dir[0]) {
        _snwprintf_s(g_path, _TRUNCATE, L"%sMWDX-render.log", dir);
        g_file = tryOpen(g_path);
    }
    if (g_file == INVALID_HANDLE_VALUE) {
        wchar_t tmp[MAX_PATH];
        if (GetTempPathW(MAX_PATH, tmp)) {
            _snwprintf_s(g_path, _TRUNCATE, L"%sMWDX-render.log", tmp);
            g_file = tryOpen(g_path);
        }
    }
    if (g_file == INVALID_HANDLE_VALUE)
    ...
    int n = std::snprintf(banner, sizeof(banner),
        "\xEF\xBB\xBF" "===== MWDX render log (DirectX 9 -> 11 / 12) =====\n"
...
const char* DecodeHR(HRESULT hr) noexcept
{
    ...
    case DXGI_ERROR_INVALID_CALL:       return "DXGI_ERROR_INVALID_CALL (illegal parameters to D3D11)";
...
void DrainInfoQueue(ID3D11Device* dev) noexcept
{
    ...
    if (FAILED(dev->QueryInterface(__uuidof(ID3D11InfoQueue),
                                   reinterpret_cast<void**>(&iq))) || !iq)
        return;
    ...
    for (UINT64 i = 0; i < n; ++i) {
        ...
        D3D11_MESSAGE* m = static_cast<D3D11_MESSAGE*>(_malloca(len));
        if (!m) continue;

        if (SUCCEEDED(iq->GetMessage(i, m, &len)) && m->pDescription &&
            m->ID != D3D11_MESSAGE_ID_QUERY_BEGIN_ABANDONING_PREVIOUS_RESULTS) {
            Level lvl = Level::Info;
            switch (m->Severity) {
            case D3D11_MESSAGE_SEVERITY_CORRUPTION:
            case D3D11_MESSAGE_SEVERITY_ERROR:   lvl = Level::Error; break;
            case D3D11_MESSAGE_SEVERITY_WARNING: lvl = Level::Warn;  break;
            default:                             lvl = Level::Info;  break;
            }
            Emit(lvl, "D3D11-debug-layer", static_cast<int>(m->ID), "InfoQueue",
                 m->pDescription);
        }
        _freea(m);
    }
