    ...
    if (indexedBlendActive || vbf == D3DVBF_TWEENING) {
        ...
        if (!s_loggedBlendGap) {
            OutputDebugStringA(indexedBlendActive
                ? "[dx9to11] FFP: indexed vertex blending (matrix palette) requested "
                  "but not implemented; falling back to WORLDMATRIX(0) only.\n"
                : "[dx9to11] FFP: D3DVBF_TWEENING requested but not implemented; "
        ...
        if (blendWeightCount > 3) {
            ...
            if (!s_loggedClamp) {
                OutputDebugStringA("[dx9to11] FFP: vertex blend with >3 explicit weights "
    ...
    for (uint32_t s = 0; s < 8; ++s) {
        ...
        case 4: {
            ...
            if (!s_loggedSphereMap) {
                OutputDebugStringA("[dx9to11] FFP: D3DTSS_TCI_SPHEREMAP requested but not "
        ...
        if (genMode != 0 && (isPosT || (!hasNormal && genMode != 2))) {
            ...
            if (!s_loggedNoNormal) {
                OutputDebugStringA(isPosT
                    ? "[dx9to11] FFP: texcoord generation requested on pre-transformed "
                      "(XYZRHW) geometry; falling back to texcoord passthrough.\n"
                    : "[dx9to11] FFP: camera-space normal/reflection texcoord generation "
