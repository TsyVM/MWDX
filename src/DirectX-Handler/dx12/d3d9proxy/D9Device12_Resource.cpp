    ...
    DWORD, BOOL Lockable, IDirect3DSurface9** ppSurface, HANDLE*)
{
    ...
    *ppSurface = nullptr;
    if (MultiSample != D3DMULTISAMPLE_NONE) {
        DXLOG_WARN("[dx12] CreateRenderTarget requested %dx MSAA, which this "
                   "backend does not implement; creating single-sampled",
                   (int)MultiSample);
    }
    ...
    DWORD, BOOL, IDirect3DSurface9** ppSurface, HANDLE*)
{
    ...
    *ppSurface = nullptr;
    if (MultiSample != D3DMULTISAMPLE_NONE) {
        DXLOG_WARN("[dx12] CreateDepthStencilSurface requested %dx MSAA, which "
                   "this backend does not implement; creating single-sampled",
                   (int)MultiSample);
    }
    D9Surface12* s = nullptr;
    const HRESULT hr = D9Surface12::CreateStandalone(
