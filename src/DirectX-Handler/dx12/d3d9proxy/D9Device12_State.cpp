...
//
// While a state block is open, this backend records the change without also
// applying it to the device -- which differs from the D3D11 backend, where the
// change is applied as well. The original runtime's behaviour here is not
// documented unambiguously, and the StateBlockRecordOnly setting exists to
// switch between the two readings.
