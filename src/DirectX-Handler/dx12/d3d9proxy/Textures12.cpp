...
D3D12_CPU_DESCRIPTOR_HANDLE D9Surface12::Dsv() noexcept
{
    if (m_backBufferIndex >= 0) return D3D12_CPU_DESCRIPTOR_HANDLE{ SIZE_T(-1) };
    return m_storage ? m_storage->Dsv(m_face, m_level)
                     : D3D12_CPU_DESCRIPTOR_HANDLE{ SIZE_T(-1) };
}
                                   ...
                                   D3D12_RESOURCE_STATES after) noexcept
{
    ...
    if (m_storage && m_storage->HasGpu())
        m_storage->Gpu().AppendTransition(out, Subresource(), after);
}
...
HRESULT STDMETHODCALLTYPE D9VolumeTexture12::GetVolumeLevel(UINT, IDirect3DVolume9** ppVolumeLevel)
{
    if (ppVolumeLevel) *ppVolumeLevel = nullptr;
    static bool s_warned = false;
    if (!s_warned) {
        s_warned = true;
        DXLOG_WARN("[dx12] GetVolumeLevel is not implemented - use LockBox/UnlockBox "
                   "for volume texture access");
    }
