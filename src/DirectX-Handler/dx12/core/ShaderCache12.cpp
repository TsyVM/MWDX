...
//
// Uses the same translator as the D3D11 backend, so both produce identical
// HLSL from a given game shader, then compiles for the D3D12 shader targets.
// Results are cached by bytecode hash plus the variant state that has to be
// compiled in.
//
